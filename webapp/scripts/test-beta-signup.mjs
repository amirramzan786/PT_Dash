...
test('marketing verification shows a dedicated confirmation screen before the app handoff', async () => {
  ...
  assert.match(html, /Create your <em>Steel account\.<\/em>/)
...
test('beta callback preserves an explicit verification marker and app confirmation flow', async () => {
  ...
  assert.match(signup, /https:\/\/app\.projectsteel\.co\.uk/)
  assert.match(signup, /url\.searchParams\.set\('beta-verified', '1'\)/)
  assert.doesNotMatch(signup, /url\.hash\s*=\s*hash/)
  assert.doesNotMatch(signup, /pt-dash\.pages\.dev|project-steel-sitepagesdev\.u1165153\.workers\.dev.*beta-verified/)
  ...
  assert.match(authGate, /Create your <span>Steel account\.<\/span>/)
...
test('Founder entitlement compatibility keeps the permanent contract valid', async () => {
  ...
  assert.match(sql, /'active', 0,\s*28, false, true, true, 'manual'/)
  assert.doesNotMatch(sql, /'active', 0,\s*0, false, true, true, 'manual'/)
  assert.match(sql, /create or replace function public\.complete_beta_verification/i)
  assert.match(sql, /create or replace function public\.reconcile_founder_entitlement/i)
  assert.match(sql, /create or replace function public\.admin_promote_waitlisted_signup/i)
  assert.match(sql, /revoke all on function public\.complete_beta_verification\(uuid, text\) from public, anon, authenticated/i)
})
...
test('membership labels are read-only, Settings-scoped, and retain Founder precedence', async () => {
  ...
  assert.match(app, /function FeedbackButton\(/)
  assert.match(app, /function FeedbackForm\(/)
  assert.doesNotMatch(app, /AlphaSupportPanel/)
  assert.doesNotMatch(app, /settings-alpha-feedback/)
  assert.match(app, /<FeedbackButton onClick=\{\(\) => openSupportPanel\('feedback'\)\}\/>/)
  assert.match(app, /supportPanel === 'feedback'/)
})
