...
test('membership labels are read-only, Settings-scoped, and retain Founder precedence', async () => {
  ...
  assert.match(app, /function ProfileMembershipBadge/)
  assert.match(app, /function MembershipTitleBadge/)
  assert.match(app, /function MembershipDetailGrid/)
  ...
  assert.match(app, /id === 'settings-membership-v5' && children\?\.props/)
  assert.match(app, /No payment method on file/)
  assert.match(app, /<ProfileMembershipBadge founderStatus=\{founderStatus\} membershipEntitlement=\{membershipEntitlement\}\/>/)
...
test('food diary keeps every core logging route available from the compact diary flow', async () => {
  ...
  assert.match(diary, /saveNutritionFoodEntry/)
  assert.match(app, /Save meal changes/)
  assert.match(app, /Edit ingredients/)
  assert.match(app, /Add food/)
