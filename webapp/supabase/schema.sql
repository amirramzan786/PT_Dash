...
create table if not exists weekly_checkins (
  ...
  weight_lb numeric(6,1) check (weight_lb > 0),
  waist_cm numeric(6,1) check (waist_cm > 0),
  chest_bust_cm numeric(6,1) check (chest_bust_cm > 0),
  hips_cm numeric(6,1) check (hips_cm > 0),
  arm_cm numeric(6,1) check (arm_cm > 0),
  thigh_cm numeric(6,1) check (thigh_cm > 0),
...
create table if not exists weekly_checkin_media (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  week_start date not null,
  media_type text not null default 'other' check (media_type in ('front','side','back','exercise_video','other')),
