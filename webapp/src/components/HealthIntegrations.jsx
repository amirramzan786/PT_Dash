...
import { getNativeRuntime, SteelHealthKit } from '../lib/nativeBridge'
...
export default function HealthIntegrations({ connections = [], onDisconnect, onDeleteImportedData, onConnect }) {
  ...
  async function connectNative(provider, name) {
    ...
    try {
      ...
      const authorization = await SteelHealthKit.requestAuthorization({ read: nativeScopes, write: [] })
      ...
      const read = await SteelHealthKit.readActivity({ start: start.toISOString(), end: end.toISOString(), timeZone })
  ...
  async function openProviderSettings(name) {
    try {
      await SteelHealthKit.openSettings()
    } catch (error) {
