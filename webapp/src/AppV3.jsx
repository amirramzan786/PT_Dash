...
export default function AppV3({ user, onSignOut }) {
  ...
  const [preferences, setPreferences] = useState({ goal: 'Lose fat and gain muscle', experienceLevel: 'Intermediate', availableEquipment: ['Machines'], trainingDays: 3, units: 'lb', limitations: '' })
  ...
  async function savePreferences(event) {
    event.preventDefault(); setSaving(true); setMessage('')
    try {
      const next = await saveProfile(user.id, { displayName: profileName.trim(), avatarUrl, ...preferences, experienceLevel: preferences.experienceLevel, availableEquipment: preferences.availableEquipment, trainingDays: Number(preferences.trainingDays), onboardingCompleted: true })
  ...
  if (!profile?.onboarding_completed) return <OnboardingFlow preferences={preferences} setPreferences={setPreferences} toggleEquipment={toggleEquipment} onComplete={savePreferences} saving={saving} onSignOut={onSignOut} />
  ...
  return <div className="steel-app">
    <main className="steel-screen">
      ...
      {tab === 'Settings' && <PreferencesForm preferences={preferences} setPreferences={setPreferences} toggleEquipment={toggleEquipment} onSubmit={savePreferences} saving={saving}/>}
