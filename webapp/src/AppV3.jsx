...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  return <div className={`page-stack nutrition-page nutrition-page-${nutritionView}`}>
    ...
    {nutritionView === 'plan' && <TodayMealPlan mealOrder={mealOrder} recipes={assignedRecipes} selectedChoices={selectedChoices} setSelectedChoices={setSelectedChoices} portions={portions} setPortions={setPortions} plannedLogs={plannedLogs} logBusy={logBusy} onEdit={(recipe) => { setFinderOpen(false); openRecipeEditor(recipe) }} onLog={logMeal} onUnlog={removeMeal} onFindFood={(meal) => { setEditingRecipe(null); setFinderMeal(meal); setFinderOpen(true) }}/>}
