...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  async function loadPlan() {
    ...
    try {
      ...
      if (plan.meals.length) setRecipes(plan.meals.map((meal) => ({ meal: (meal.meal_type || 'MEAL').toUpperCase(), name: meal.title, detail: meal.description || 'Assigned recipe details', calories: meal.calories || 0, protein: meal.protein_g || 0, carbs: meal.carbs_g || 0, fat: meal.fat_g || 0, servingG: meal.serving_g || 0, ingredients: meal.description ? [meal.description] : ['Ingredients will be added to this assigned recipe.'], instructions: 'Use the ingredients above and follow the preparation notes supplied with this assigned meal.' })))
    } catch (error) {
  ...
  async function toggleMeal(recipe) {
    const existing = loggedMeals.find((row) => row.meal_type === recipe.meal)
    const isLogged = Boolean(existing)
    setLoggedMeals((current) => isLogged ? current.filter((row) => row.meal_type !== recipe.meal) : [...current, { meal_type: recipe.meal, recipe_name: recipe.name, calories: recipe.calories, protein_g: recipe.protein, carbs_g: recipe.carbs, fat_g: recipe.fat, serving_g: recipe.servingG }])
    try {
      if (isLogged) await deleteMealLog({ userId, mealDate: today, mealType: recipe.meal })
      else await saveMealLog({ userId, mealDate: today, mealType: recipe.meal, recipeName: recipe.name, calories: recipe.calories, protein: recipe.protein, carbs: recipe.carbs, fat: recipe.fat, servingG: recipe.servingG })
    } catch (error) {
      setLoggedMeals((current) => isLogged ? [...current, existing] : current.filter((row) => row.meal_type !== recipe.meal))
    }
  }
