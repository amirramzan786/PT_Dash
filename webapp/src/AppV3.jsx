...
import {
  ...
  Footprints, Home, LineChart, LogOut, Play, RotateCcw, Save, Scale, Settings,
  ...
  return <div className="steel-app">
    <main className="steel-screen">
      <header className="v2-topbar">
        <div className="brand-lockup"><div className="brand-emblem">S</div><div><div className="eyebrow">SPARTAN STRENGTH, EVERY DAY</div><h1>PROJECT <span>STEEL</span></h1></div></div>
      ...
      {tab === 'Home' && <div className="page-stack home-page-stack">
        <section className="v4-welcome-card"><div className="v4-hero-art"><div className="v4-hero-figure" aria-hidden="true" /><button type="button" className="hero-step-counter" style={{ '--step-progress': `${stepProgress}%` }} onClick={() => setTab('Progress')} aria-label={`Today’s steps: ${todaySteps.toLocaleString('en-GB')}. Open Progress`}><span className="hero-step-ring"><Footprints size={16}/><strong>{todaySteps.toLocaleString('en-GB')}</strong><small>today’s steps</small></span></button></div><div className="v4-hero-copy"><span className="eyebrow">WELCOME BACK,</span><h2>{firstName}</h2><p>You’ve got this. Let’s build something strong today.</p></div><div className="v4-metric-grid"><article><Dumbbell size={17}/><span>WORKOUTS</span><strong>{stats.sessionCount}</strong><small>Logged</small></article><article><Flame size={17}/><span>STREAK</span><strong>{stats.streakDays || 0}</strong><small>Days</small></article><article><Activity size={17}/><span>VOLUME</span><strong>{stats.volumeKg ? Math.round(stats.volumeKg).toLocaleString('en-GB') : 0}</strong><small>kg lifted</small></article><button type="button" className="v4-metric-link" onClick={() => setTab('Weight')}><Scale size={17}/><span>WEIGHT</span><strong>{latestWeight ? latestWeight.toFixed(1) : '—'}</strong><small>lb</small></button></div></section>
