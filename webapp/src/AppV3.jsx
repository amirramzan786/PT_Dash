...
function DietPlanOverview({ mealOrder, recipes, selectedChoices, dietary, target }) {
  return <section className="diet-plan-overview">
    <div className="section-heading"><div><span className="eyebrow">YOUR DIET PLAN</span><h3>The structure behind your day.</h3></div><span className="nutrition-progress">{dietary}</span></div>
    <article className="diet-plan-target"><div><span className="eyebrow">DAILY DIRECTION</span><strong>{target.calories.toLocaleString('en-GB')} kcal · {target.protein_g}g protein</strong><small>Built around {mealOrder.length} planned meals. Adjust what you eat in Food Diary.</small></div></article>
    <div className="diet-plan-meals">{mealOrder.map((meal) => { const options = recipes.filter((recipe) => recipe.meal === meal); const selected = options.find((recipe) => recipe.optionKey === selectedChoices[meal]) || options[0]; return <article key={meal} className="diet-plan-meal"><span className="eyebrow">{meal}</span><h4>{selected.name}</h4><p>{selected.detail}</p><small>{(selected.ingredients || []).map(ingredientLabel).join(' · ')}</small><footer><span>{selected.calories} kcal</span><span>{selected.protein}g protein</span></footer></article> })}</div>
    <p className="diet-plan-note">Your plan is a useful default, not a rulebook. Use Food Diary to log the meal and serving you actually have.</p>
  </section>
}
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  return <div className={`page-stack nutrition-page nutrition-page-${nutritionView}`}>
    <section className="page-intro"><span className="eyebrow">FUEL YOUR GOAL</span><h2>{nutritionView === 'diary' ? 'Food diary' : nutritionView === 'plan' ? 'Your nutrition plan' : 'Recipe library'}</h2><p>{nutritionView === 'diary' ? 'Choose your planned meal, adjust it for real life and log what you actually eat.' : nutritionView === 'plan' ? 'Your daily targets and meal structure in one calm overview.' : `Browse the recipes assigned around your ${dietary === 'No preference' ? 'training plan' : dietary.toLowerCase()} preferences.`}</p><div className="nutrition-view-switcher" role="tablist" aria-label="Nutrition views"><button type="button" className={nutritionView === 'diary' ? 'active' : ''} onClick={() => setNutritionView('diary')}>Food diary</button><button type="button" className={nutritionView === 'plan' ? 'active' : ''} onClick={() => setNutritionView('plan')}>Your plan</button><button type="button" className={nutritionView === 'library' ? 'active' : ''} onClick={() => setNutritionView('library')}>Recipes</button></div></section>
    ...
    {nutritionView === 'plan' && <DietPlanOverview mealOrder={mealOrder} recipes={assignedRecipes} selectedChoices={selectedChoices} dietary={dietary} target={target}/>}
