...
import {
  ...
  Send, ShieldCheck, Sparkles, PersonStanding, Target, Trash2, UserRound, Watch, ListChecks, ClipboardCheck, X,
} from 'lucide-react'
import {
...
import { buildAiCoachInsight, buildDailySummary, buildTrainingRecommendation, dailyQuote } from './lib/homeGuidance'
import { sevenDayStepAverage, stepGoalProgress } from './lib/steps'
import { buildWeeklyNutritionAdherence, buildWeightTrend, nutritionTargetForDate, startOfWeek } from './lib/progressSignals'
import './app-v2.css'
import './settings.css'
import './v3.css'
...
import { buildSnackChoices, snackTasteOptions } from './lib/snackChoices'
import './recovery.css'
import './coach.css'
...
function AiCoachInsightCard({ insight, onNavigate, hasWorkout }) {
  const [choice, setChoice] = useState('')
  if (!insight) return null
  return <section className="ai-coach-insight-card" aria-label="Steel AI Coach insight"><div className="ai-coach-insight-heading"><div><span className="eyebrow">STEEL AI COACH · INSIGHT ONLY</span><h3>{insight.title}</h3></div><Sparkles size={20}/></div><p className="ai-coach-observation">{insight.observation}</p><p className="ai-coach-uncertainty">{insight.uncertainty}</p><div className="ai-coach-question"><strong>{insight.question}</strong><div className="ai-coach-options">{insight.options.map((option) => <button type="button" key={option} className={choice === option ? 'selected' : ''} onClick={() => setChoice(option)}>{option}</button>)}</div></div>{choice && <div className="ai-coach-choice" role="status"><Check size={16}/><span>{choice === 'Not now' ? 'No change made. You can revisit this insight later.' : `${choice} selected for discussion. Steel has not changed your plan.`}</span></div>}<button type="button" className="ai-coach-checkin-link" onClick={() => onNavigate('Checkin')}>Review weekly check-in <ChevronRight size={14}/></button>{!hasWorkout && <small className="ai-coach-disclaimer">Add a planned workout when you are ready; this insight never creates one automatically.</small>}</section>
}
...
function ProgressPage({ stats, sessions, weights, stepHistory, totalSteps }) {
  ...
  const weightTrend = buildWeightTrend(weights)
...
function PreferencesForm({ preferences, setPreferences, toggleEquipment, onSubmit, saving, planChangeStatus, onRequestPlanReview }) {
  return <form className="steel-card preferences-form" onSubmit={onSubmit}>
    <div className="settings-form-heading"><div><span className="eyebrow">TRAINING PREFERENCES</span><h3>Shape your Steel plan</h3></div><Target size={20}/></div>
    <p className="settings-form-copy">These preferences give your future AI trainer the context to make better recommendations.</p>
    <label><span>Primary goal</span><select value={preferences.goal} onChange={e=>setPreferences({...preferences, goal:e.target.value})}><option>Lose fat and gain muscle</option><option>Build muscle</option><option>Get stronger</option><option>Improve fitness</option><option>Train consistently</option></select></label>
    <fieldset><legend>Experience level</legend><div className="preference-choice-grid">{experienceOptions.map(option=><button type="button" key={option} className={preferences.experienceLevel===option?'selected':''} aria-pressed={preferences.experienceLevel===option} onClick={()=>setPreferences({...preferences, experienceLevel:option})}>{option}</button>)}</div></fieldset>
    <fieldset><legend>Available equipment</legend><div className="equipment-choice-grid">{equipmentOptions.map(option=><button type="button" key={option} className={preferences.availableEquipment.includes(option)?'selected':''} aria-pressed={preferences.availableEquipment.includes(option)} onClick={()=>toggleEquipment(option)}>{preferences.availableEquipment.includes(option)?'✓ ':''}{option}</button>)}</div><small className="field-hint">Select everything you can use. Keep at least one option selected.</small></fieldset>
    <div className="preference-split"><label><span>Training days per week</span><select value={preferences.trainingDays} onChange={e=>setPreferences({...preferences, trainingDays:Number(e.target.value)})}>{[1,2,3,4,5,6,7].map(day=><option key={day} value={day}>{day} {day===1?'day':'days'}</option>)}</select></label><label><span>Weekly check-in day</span><select value={preferences.checkinDay} onChange={e=>setPreferences({...preferences, checkinDay:Number(e.target.value)})}>{checkinDays.map(([day,value])=><option key={day} value={value}>{day}</option>)}</select></label></div>
    <div className="preference-split"><label><span>Weight units</span><select value={preferences.units} onChange={e=>setPreferences({...preferences, units:e.target.value})}><option value="lb">Pounds (lb)</option><option value="kg">Kilograms (kg)</option></select></label><label><span>Daily step goal</span><input type="number" inputMode="numeric" min="1000" max="100000" step="500" value={preferences.dailyStepGoal} onChange={e=>setPreferences({...preferences, dailyStepGoal:e.target.value})}/><small className="field-hint">You can change this at any time.</small></label></div>
    <label><span>Injuries, limitations or anything Steel should know</span><textarea value={preferences.limitations} onChange={e=>setPreferences({...preferences, limitations:e.target.value})} placeholder="Optional — for example, shoulder discomfort or a movement to avoid." rows="3"/></label>
    <div className="settings-form-divider"><span className="eyebrow">MEAL PREFERENCES</span><strong>Give future meal plans the right guardrails.</strong></div>
    <label><span>Dietary preference</span><select value={preferences.dietaryPreference} onChange={e=>setPreferences({...preferences, dietaryPreference:e.target.value})}>{dietaryOptions.map(option=><option key={option}>{option}</option>)}</select></label>
    <div className="preference-split"><label><span>Preferred meals per day</span><select value={preferences.mealsPerDay} onChange={e=>setPreferences({...preferences, mealsPerDay:Number(e.target.value)})}>{[2,3,4,5,6].map(meals=><option key={meals} value={meals}>{meals} meals</option>)}</select></label><label><span>Allergies or intolerances</span><input value={preferences.allergies} onChange={e=>setPreferences({...preferences, allergies:e.target.value})} placeholder="Optional"/></label></div>
    <SnackPreferenceFields preferences={preferences} setPreferences={setPreferences}/>
    <button className="gold-button" disabled={saving}><Save size={17}/> {saving?'Saving…':'Save preferences'}</button>
    <aside className="programme-review-card"><div><span className="eyebrow">PROGRAMME REVIEW</span><strong>{planChangeStatus?.can_change_now ? 'Ready when you are' : 'Your plan is settling in'}</strong><p>{planChangeStatus?.can_change_now ? 'Use your one 28-day review to rebuild your Steel programme around your updated preferences.' : planChangeStatus?.next_change_at ? `Your next programme review opens ${formatDate(planChangeStatus.next_change_at)}.` : 'Your first programme is created during onboarding. Changes are then available every 28 days.'}</p></div><button type="button" className="text-link" disabled={!planChangeStatus?.can_change_now || saving} onClick={onRequestPlanReview}>Review programme <ChevronRight size={15}/></button></aside>
  </form>
}
...
function mergeAiPreferences(preferences, profile) {
  ...
  if (typeof profile?.snacksEnabled === 'boolean') next.snacksEnabled = profile.snacksEnabled
  if (Array.isArray(profile?.snackPreferences)) next.snackPreferences = profile.snackPreferences
  return next
}
...
function SnackPreferenceFields({ preferences, setPreferences }) {
  const selected = Array.isArray(preferences.snackPreferences) ? preferences.snackPreferences : []
  const toggleSnackTaste = (taste) => setPreferences({ ...preferences, snackPreferences: selected.includes(taste) ? selected.filter((item) => item !== taste) : [...selected, taste] })
  return <fieldset className="snack-preference-fields"><legend>Snacks</legend><label><span>Would you like a snack section in your food diary?</span><select value={preferences.snacksEnabled ? 'yes' : 'no'} onChange={(event) => setPreferences({ ...preferences, snacksEnabled: event.target.value === 'yes' })}><option value="yes">Yes — give me snack choices</option><option value="no">No snack section for now</option></select></label>{preferences.snacksEnabled && <><span className="field-label">What do you genuinely enjoy snacking on?</span><div className="equipment-choice-grid">{snackTasteOptions.map((taste) => <button type="button" key={taste} className={selected.includes(taste) ? 'selected' : ''} aria-pressed={selected.includes(taste)} onClick={() => toggleSnackTaste(taste)}>{selected.includes(taste) ? '✓ ' : ''}{taste}</button>)}</div><small className="field-hint">Choose any that fit. Steel uses them to prioritise three practical snack choices; you can still search and add anything else.</small></>}</fieldset>
}

function OnboardingFoodContextFields({ preferences, setPreferences }) {
  return <><label><span>How much time do you usually have to cook?</span><select value={preferences.cookingTime} onChange={(event) => setPreferences({ ...preferences, cookingTime: event.target.value })}><option value="">Prefer not to say</option>{['Minimal', '15–30 minutes', '30–60 minutes', 'Enjoy cooking'].map((option) => <option key={option}>{option}</option>)}</select></label><label><span>Foods or meals you genuinely enjoy</span><textarea value={preferences.preferredFoods} onChange={(event) => setPreferences({ ...preferences, preferredFoods: event.target.value })} placeholder="Optional — tell us cuisines, staple foods or meals you would be happy to repeat." rows="3"/><small className="field-hint">Steel will use this to offer practical meal choices rather than a rigid, generic menu.</small></label><SnackPreferenceFields preferences={preferences} setPreferences={setPreferences}/></>
}
...
function InlineMealIngredientEditor({ userId, recipe, components, matchingComponentIndex, onUpdateComponent, onRemoveComponent, onMatchComponent, onAddComponent, onReceiveFood, onSave, saving, saveError, onClose }) {
  return <section className="meal-component-editor meal-component-editor-inline"><div className="meal-component-editor-heading"><div><span className="eyebrow">EDIT {recipe.meal}</span><h3>{recipe.name}</h3><p>Adjust the ingredients in this meal, then save it back to your plan.</p></div><button type="button" className="text-link" onClick={onClose}>Close</button></div><div className="meal-component-list">{components.map((component, index) => { const totals = componentTotals([component]); return <article key={component.key || index} className={component.foodId ? 'meal-component is-linked' : 'meal-component'}><div><strong>{component.name || 'Unnamed ingredient'}</strong><small>{component.foodId ? `${Math.round(totals.calories)} kcal · ${Math.round(totals.protein)}g protein` : 'Match this ingredient to a catalogue food for exact macros.'}</small></div><label><span>Amount (g)</span><input type="number" min="1" inputMode="decimal" value={component.grams || ''} onChange={(event) => onUpdateComponent(index, { grams: event.target.value, servingLabel: `${event.target.value}g` })}/></label><button type="button" onClick={() => onMatchComponent(index)}>{component.foodId ? 'Change food' : 'Match food'}</button><button type="button" aria-label={`Remove ${component.name || 'ingredient'}`} onClick={() => onRemoveComponent(index)}><Trash2 size={15}/></button></article> })}</div>{saveError && <p className="meal-component-save-error" role="alert">{saveError}</p>}<div className="meal-component-actions"><button type="button" className="text-link" onClick={onAddComponent}><Search size={15}/> Add another food</button><button type="button" className="gold-button" disabled={saving} onClick={onSave}>{saving ? 'Saving…' : 'Save meal changes'}</button></div>{matchingComponentIndex !== null && <div className="meal-component-picker"><div><span className="eyebrow">{components[matchingComponentIndex]?.name ? `MATCH ${components[matchingComponentIndex].name.toUpperCase()}` : 'ADD TO THIS MEAL'}</span><p>Search the catalogue or scan a barcode. The chosen food is added to this meal.</p></div><FoodDiary compact userId={userId} mealLogs={[]} initialMeal={recipe.meal} onFoodPicked={onReceiveFood}/></div>}</section>
}
function TodayMealPlan({ mealOrder, recipes, selectedChoices, setSelectedChoices, portions, setPortions, plannedLogs, logBusy, onEdit, onLog, onUnlog, onFindFood, editor, finder }) {
  return <section className="today-meal-plan">
    <div className="section-heading"><div><span className="eyebrow">YOUR MEALS FOR TODAY</span><h3>Choose. Adjust. Log.</h3></div><span className="nutrition-progress">3 options per meal</span></div>
    {mealOrder.map((meal) => {
      const options = recipes.filter((recipe) => recipe.meal === meal)
      const selected = options.find((recipe) => recipe.optionKey === selectedChoices[meal]) || options[0]
      const portion = Number(portions[meal] || 1)
      const existing = plannedLogs.find((entry) => entry.meal_type === meal)
      return <article className={existing ? 'today-meal-card is-logged' : 'today-meal-card'} key={meal}>
        <div className="today-meal-heading"><span className="eyebrow">{meal}</span>{existing && <span className="meal-logged-label">LOGGED</span>}</div>
        <label className="today-meal-choice"><span>Meal option</span><select value={selected.optionKey} onChange={(event) => setSelectedChoices({ ...selectedChoices, [meal]: event.target.value })}>{options.map((option) => <option key={option.optionKey} value={option.optionKey}>{option.name} · {option.calories} kcal · {option.protein}g protein</option>)}</select></label>
        ...
        {finder?.meal === meal && <section className="inline-food-finder today-meal-food-finder"><div><span className="eyebrow">ADD TO {meal}</span><h3>Find a food</h3><p>Search the catalogue or scan a barcode, then choose the serving you actually had.</p></div><FoodDiary compact userId={finder.userId} mealLogs={[]} initialMeal={meal} onFoodPicked={finder.onFoodPicked}/><button type="button" className="text-link" onClick={finder.onClose}>Close food finder</button></section>}
      </article>
    })}
  </section>
}
function DietPlanOverview({ mealOrder, recipes, dietary, target, onOpenDiary }) {
  return <section className="diet-plan-overview">
    <div className="section-heading"><div><span className="eyebrow">YOUR DIET PLAN</span><h3>All meals assigned to you.</h3></div><span className="nutrition-progress">{recipes.length} recipes</span></div>
    <article className="diet-plan-target"><div><span className="eyebrow">DAILY DIRECTION</span><strong>{target.calories.toLocaleString('en-GB')} kcal · {target.protein_g}g protein</strong><small>{mealOrder.length} meal moments, with three assigned recipes for each. Food Diary is where you choose, adjust and log what you actually eat.</small></div><button type="button" className="text-link" onClick={onOpenDiary}>Open food diary <ChevronRight size={14}/></button></article>
    <div className="diet-plan-meals">{mealOrder.map((meal) => {
      const options = recipes.filter((recipe) => recipe.meal === meal)
      return <section key={meal} className="diet-plan-meal-group"><div className="diet-plan-meal-group-heading"><span className="eyebrow">{meal}</span><small>{options.length} assigned recipes</small></div><div className="diet-plan-recipe-list">{options.map((recipe) => <details key={recipe.optionKey} className="diet-plan-recipe"><summary><span><strong>{recipe.name}</strong><small>{recipe.detail}</small></span><span className="diet-plan-recipe-summary"><em>{recipe.calories} kcal</em><ChevronDown size={16}/></span></summary><div className="diet-plan-recipe-content"><div className="diet-plan-recipe-macros"><span>{recipe.protein}g protein</span><span>{recipe.carbs}g carbs</span><span>{recipe.fat}g fat</span><span>{recipe.servingG}g serving</span></div><div><strong>Ingredients</strong><p>{(recipe.ingredients || []).map(ingredientLabel).join(' · ')}</p></div><div><strong>How to make it</strong><p>{recipe.instructions}</p></div></div></details>)}</div></section>
    })}</div>
    <p className="diet-plan-note">This is your plan, not a rulebook. Choose the option that fits today, then make it honest in Food Diary.</p>
  </section>
}
function NutritionWeekInsight({ summary }) {
  const headline = summary.adherencePercent === null
    ? 'Your first weekly score is ready tomorrow.'
    : `${summary.adherencePercent}% on target across completed days`
  const detail = summary.scoredDays
    ? `${summary.onTargetDays} of ${summary.scoredDays} completed days landed within your calorie range and protein target.`
    : 'Today is in progress, so Steel will not score it early.'
  return <section className="nutrition-weekly-insight steel-card">
    <div className="nutrition-weekly-heading"><div><span className="eyebrow">THIS WEEK / FUEL</span><h3>{headline}</h3></div><span>{summary.loggedDays}/{summary.elapsedDays} logged</span></div>
    <p>{detail}</p>
    <div className="nutrition-weekly-days" aria-label="Daily nutrition status">{summary.days.map((day) => <div className={`nutrition-weekly-day is-${day.state}${day.onTarget ? ' is-on-target' : ''}`} key={day.date} title={`${day.date}: ${day.logs ? `${Math.round(day.calories)} kcal and ${Math.round(day.protein)}g protein logged` : day.state === 'upcoming' ? 'Upcoming' : 'No food logged'}`}><b>{day.shortLabel}</b><i aria-hidden="true"/></div>)}</div>
    <small>On target means within 10% of your daily calorie target and at least 90% of your protein target. Future days are never counted.</small>
  </section>
}
...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  // Kept while legacy saved links settle; the Recipes route is no longer reachable from navigation.
  ...
  const starterChoices = {
    ...
    SNACK: buildSnackChoices(preferences.snackPreferences),
  }
  ...
  async function saveEditedRecipe() {
    ...
    if (!editingComponents.length || editingComponents.some((item) => !String(item.name || '').trim() || Number(item.grams) <= 0)) {
      setRecipeSaveError('Add a name and a valid amount for every ingredient before saving.')
      return
    }
    ...
    try {
      ...
      setRecipeSaveError(error?.message || 'Steel could not save those meal changes. Please try again.')
  ...
  async function loadPlan() {
    ...
    try {
      ...
      if (plan.target) setTarget({
        calories: plan.target.calories || 2050,
        protein_g: plan.target.protein_g || 170,
        training_day_indices: Array.isArray(plan.target.training_day_indices) ? plan.target.training_day_indices : [],
        training_calories: plan.target.training_calories || null,
        training_protein_g: plan.target.training_protein_g || null,
        rest_calories: plan.target.rest_calories || null,
        rest_protein_g: plan.target.rest_protein_g || null,
      })
      const templates = Object.entries(starterChoices).flatMap(([meal, options]) => options.map((option, index) => ({ ...option, meal, optionKey: `${meal.toLowerCase()}-${index + 1}`, optionNumber: index + 1, sortOrder: index + 1 })))
      const incomplete = templates.filter((template) => { const existing = plan.meals.find((item) => String(item.meal_type || '').toUpperCase() === template.meal && Number(item.option_number || 1) === template.optionNumber); return !existing || !Array.isArray(existing.ingredients) || existing.ingredients.length === 0 })
      const repaired = await Promise.all(incomplete.map((template) => { const existing = plan.meals.find((item) => String(item.meal_type || '').toUpperCase() === template.meal && Number(item.option_number || 1) === template.optionNumber); return saveMealPlanItem({ userId, item: { ...template, id: existing?.id } }) }))
      const repairedIds = new Set(repaired.map((item) => item.id))
      setRecipes(normalisePlan([...plan.meals.filter((item) => !repairedIds.has(item.id)), ...repaired]))
    } catch (error) {
  ...
  async function refreshLoggedMeals() {
    const [todayRows, weekRows] = await Promise.all([
      getMealLogs(userId, today, today),
      getMealLogs(userId, weekStart, today),
    ])
    setLoggedMeals(todayRows)
    setWeeklyMeals(weekRows)
  }
  ...
  const todayTarget = nutritionTargetForDate(target, today)
  const macroTargets = { protein: todayTarget.protein_g || 170, fat: Math.round((todayTarget.calories * .25) / 9), carbs: Math.max(0, Math.round((todayTarget.calories - (todayTarget.protein_g || 170) * 4 - Math.round((todayTarget.calories * .25) / 9) * 9) / 4)) }
  const calorieProgress = Math.min(100, Math.round((loggedCalories / Math.max(1, todayTarget.calories)) * 100))
  const weeklyMealsWithToday = [...weeklyMeals.filter((row) => row.meal_date !== today), ...loggedMeals]
  const weeklyNutrition = buildWeeklyNutritionAdherence(weeklyMealsWithToday, target, today)
  const coreMealOrder = ['BREAKFAST', 'LUNCH', 'DINNER'].filter((meal) => assignedRecipes.some((recipe) => recipe.meal === meal)).slice(0, Math.max(3, Number(preferences.mealsPerDay) || 3))
  const mealOrder = preferences.snacksEnabled !== false && assignedRecipes.some((recipe) => recipe.meal === 'SNACK') ? [...coreMealOrder, 'SNACK'] : coreMealOrder
  ...
  return <div className={`page-stack nutrition-page nutrition-page-${nutritionView}`}>
    <section className="page-intro"><span className="eyebrow">FUEL YOUR GOAL</span><h2>{nutritionView === 'diary' ? 'Food diary' : 'Your nutrition plan'}</h2><p>{nutritionView === 'diary' ? 'Choose your planned meal, adjust it for real life and log what you actually eat.' : `Every recipe assigned around your ${dietary === 'No preference' ? 'training plan' : dietary.toLowerCase()} preferences, in one calm overview.`}</p><div className="nutrition-view-switcher" role="tablist" aria-label="Nutrition views"><button type="button" role="tab" aria-selected={nutritionView === 'diary'} className={nutritionView === 'diary' ? 'active' : ''} onClick={() => setNutritionView('diary')}>Food diary</button><button type="button" role="tab" aria-selected={nutritionView === 'plan'} className={nutritionView === 'plan' ? 'active' : ''} onClick={() => setNutritionView('plan')}>Your plan</button></div></section>
    ...
    {(nutritionView === 'diary' || nutritionView === 'plan') && <section className="nutrition-intake-dashboard"><div className="nutrition-dashboard-top"><div className="macro-orb" style={{ '--progress': `${calorieProgress}%` }}><span>{loggedCalories.toLocaleString('en-GB')}</span><small>kcal logged</small></div><div className="nutrition-dashboard-copy"><span className="eyebrow">{todayTarget.kind === 'training' ? 'TRAINING DAY TARGET' : todayTarget.kind === 'rest' ? 'REST DAY TARGET' : 'TODAY’S TARGET'}</span><h3>{todayTarget.calories.toLocaleString('en-GB')} kcal · {todayTarget.protein_g}g protein</h3><p>{todayTarget.kind === 'standard' ? dietary : `Assigned by your trainer · ${dietary}`} · {preferences.mealsPerDay} meals per day{preferences.snacksEnabled !== false ? ' + snacks' : ''}</p></div><span className="nutrition-kcal-remaining">{Math.max(0, todayTarget.calories - loggedCalories).toLocaleString('en-GB')} kcal remaining</span></div><div className="nutrition-macro-grid">{[['Protein', loggedProtein, macroTargets.protein, 'protein'], ['Carbs', loggedCarbs, macroTargets.carbs, 'carbs'], ['Fat', loggedFat, macroTargets.fat, 'fat']].map(([label, value, goal, key]) => <div key={key}><span>{label}</span><strong>{Math.round(value)}g <small>/ {goal}g</small></strong><i><b style={{ width: `${Math.min(100, Math.round((value / Math.max(1, goal)) * 100))}%` }}/></i></div>)}</div></section>}
    {nutritionView === 'diary' && <NutritionWeekInsight summary={weeklyNutrition}/>}
    {nutritionView === 'diary' && <TodayMealPlan mealOrder={mealOrder} recipes={assignedRecipes} selectedChoices={selectedChoices} setSelectedChoices={setSelectedChoices} portions={portions} setPortions={setPortions} plannedLogs={plannedLogs} logBusy={logBusy} onEdit={(recipe) => { setFinderOpen(false); openRecipeEditor(recipe) }} onLog={logMeal} onUnlog={removeMeal} onFindFood={(meal) => { setEditingRecipe(null); setFinderMeal(meal); setFinderOpen(true) }} editor={editingRecipe ? { userId, recipe: editingRecipe, components: editingComponents, matchingComponentIndex, onUpdateComponent: updateEditingComponent, onRemoveComponent: (index) => setEditingComponents((items) => items.filter((_, itemIndex) => itemIndex !== index)), onMatchComponent: setMatchingComponentIndex, onAddComponent: () => setMatchingComponentIndex(-1), onReceiveFood: receiveCatalogueFood, onSave: saveEditedRecipe, saving: savingRecipe, saveError: recipeSaveError, onClose: () => { setEditingRecipe(null); setMatchingComponentIndex(null); setRecipeSaveError('') } } : null} finder={finderOpen ? { meal: finderMeal, userId, onFoodPicked: addFoundFood, onClose: () => setFinderOpen(false) } : null}/>}
    {nutritionView === 'plan' && <DietPlanOverview mealOrder={mealOrder} recipes={assignedRecipes} dietary={dietary} target={todayTarget} onOpenDiary={() => setNutritionView('diary')}/>}
...
function ExerciseLibrary({ onSelectExercise }) {
  ...
  function toggleFilter(value, setFilter) { setFilter((selected) => selected.includes(value) ? selected.filter((item) => item !== value) : [...selected, value]) }
  const activeFilterCount = muscleGroups.length + equipment.length + difficulties.length
  const visibleRows = rows.filter((row) => {
    const text = `${row.name} ${row.primary_muscle_group} ${(row.secondary_muscle_groups ?? []).join(' ')}`.toLowerCase()
    return (!query.trim() || text.includes(query.trim().toLowerCase())) && (!muscleGroups.length || muscleGroups.includes(row.primary_muscle_group)) && (!equipment.length || (row.equipment ?? []).some((item) => equipment.includes(item))) && (!difficulties.length || difficulties.includes(row.difficulty))
  })
  return <section className="exercise-library"><div className="library-toolbar"><label className="library-search"><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search exercises" aria-label="Search exercises"/></label><div className="library-filter-groups"><div className="library-filter-group"><span>Muscle</span><div>{muscles.map((muscle) => <button type="button" key={muscle} className={muscleGroups.includes(muscle) ? 'active' : ''} aria-pressed={muscleGroups.includes(muscle)} onClick={() => toggleFilter(muscle, setMuscleGroups)}>{muscle}</button>)}</div></div><div className="library-filter-group"><span>Equipment</span><div>{equipmentOptions.map((item) => <button type="button" key={item} className={equipment.includes(item) ? 'active' : ''} aria-pressed={equipment.includes(item)} onClick={() => toggleFilter(item, setEquipment)}>{item}</button>)}</div></div><div className="library-filter-group"><span>Level</span><div>{difficultyOptions.map((level) => <button type="button" key={level} className={difficulties.includes(level) ? 'active' : ''} aria-pressed={difficulties.includes(level)} onClick={() => toggleFilter(level, setDifficulties)}>{level}</button>)}</div></div>{activeFilterCount ? <button type="button" className="library-clear-filters" onClick={() => { setMuscleGroups([]); setEquipment([]); setDifficulties([]) }}>Clear filters</button> : null}</div></div>{busy ? <div className="library-state"><span>Loading the exercise library…</span></div> : error ? <div className="library-state library-error"><strong>We couldn’t load the library.</strong><span>Check your connection and try again.</span><button type="button" className="library-retry" onClick={refreshLibrary}>Try again</button></div> : <><div className="library-result-count">{visibleRows.length} exercise{visibleRows.length === 1 ? '' : 's'} available{activeFilterCount ? ` · ${activeFilterCount} filter${activeFilterCount === 1 ? '' : 's'} active` : ''}</div><div className="exercise-library-list">{visibleRows.map((row) => <article className="exercise-library-card" key={row.id}><div className="library-exercise-icon"><Dumbbell size={19}/></div><div className="library-exercise-copy"><strong>{row.name}</strong><span>{row.primary_muscle_group} · {(row.equipment ?? []).join(' / ') || 'Gym'}</span><small>{row.difficulty || 'All levels'} · {row.movement_pattern || 'Strength'}</small></div><div className="library-card-actions">{onSelectExercise && <button type="button" className="library-use-button" onClick={() => onSelectExercise(row)}>Use this exercise</button>}<button type="button" className="library-details-button" onClick={() => setSelectedExercise(row)}>Details</button>{row.video_url ? <a className="library-video-link" href={row.video_url} target="_blank" rel="noreferrer">Form <ExternalLink size={13}/></a> : null}</div></article>)}{!visibleRows.length && <div className="library-state"><strong>No exercises match those filters.</strong><span>Try a different muscle group, equipment type or difficulty.</span></div>}</div></>}{selectedExercise && <ExerciseDetailPanel exercise={selectedExercise} onClose={() => setSelectedExercise(null)}/>}</section>
}
...
function PlanPage({ workouts, openWorkout, onLogWorkout, onEditWorkout, onDuplicateWorkout, libraryMode, onSelectExercise, onCancelLibrary }) {
  ...
  return <div className="page-stack"><section className="page-intro">{libraryMode && <button type="button" className="settings-back-button" onClick={onCancelLibrary}><ArrowLeft size={17}/> Back to active workout</button>}<span className="eyebrow">YOUR TRAINING</span><h2>{view === 'programme' ? 'Workouts' : libraryMode ? 'Choose a replacement' : 'Exercise library'}</h2><p>{view === 'programme' ? 'Your plan is ready. Start the next session, or open any session to make it yours.' : libraryMode ? 'Choose any movement that suits you today. Your session stays yours.' : 'Find the right movement without leaving your training flow.'}</p><div className="page-switcher" role="tablist" aria-label="Workouts and exercise library"><button type="button" className={view === 'programme' ? 'active' : ''} onClick={() => setView('programme')}>My workouts</button><button type="button" className={view === 'library' ? 'active' : ''} onClick={() => setView('library')}>Exercise library</button></div></section>{view === 'programme' ? <>{nextWorkout && <section className="next-session-card"><div><span className="eyebrow">UP NEXT</span><h3>{nextWorkout.name}</h3><p>{nextWorkout.exercises.length} exercises · {nextWorkout.duration}{nextWorkout.focus ? ` · ${nextWorkout.focus}` : ''}</p></div><button type="button" className="gold-button" onClick={() => openWorkout(nextWorkout)}><Play size={16}/> Start</button></section>}<button type="button" className="log-workout-cta" onClick={onLogWorkout}><Play size={17}/><span><strong>Log a different workout</strong><small>Build today’s session from the exercise library</small></span><ChevronRight size={17}/></button>{workouts.length ? <section className="programme-session-list"><div className="section-heading"><div><span className="eyebrow">YOUR PROGRAMME</span><h3>{workouts.length} planned session{workouts.length === 1 ? '' : 's'}</h3></div></div>{workouts.map((workout, index) => <details className="programme-session" key={workout.id}><summary><span className="programme-session-number">{String(index + 1).padStart(2, '0')}</span><span><strong>{workout.name}</strong><small>{workout.exercises.length} exercises · {workout.duration}</small></span><ChevronDown size={17}/></summary><div className="programme-session-content"><div className="programme-session-actions"><button type="button" className="gold-button" onClick={() => openWorkout(workout)}><Play size={15}/> Start session</button>{workout.source !== 'catalog' && <><button type="button" onClick={() => onEditWorkout(workout)}>Edit</button><button type="button" onClick={() => onDuplicateWorkout(workout)}>Duplicate</button></>}</div><div className="compact-exercise-list">{workout.exercises.map((exercise, exerciseIndex) => <div className="compact-exercise" key={exercise.id}><span>{exerciseIndex + 1}</span><div><strong>{exercise.name}</strong><small>{exercise.muscleGroup ? `${exercise.muscleGroup} · ` : ''}{exercise.equipment} · {exercise.sets} × {exercise.reps}</small></div></div>)}</div><div className="finisher-badge"><Flame size={15}/> {workout.finisher}</div></div></details>)}</section> : <div className="library-state plan-empty-state"><strong>No saved workouts yet.</strong><span>Build a session from the exercise library to get started.</span><button type="button" className="library-retry" onClick={onLogWorkout}>Create a workout</button></div>}</> : <ExerciseLibrary onSelectExercise={onSelectExercise}/>}</div>
}
...
function ProfileMembershipBadge({ founderStatus, membershipEntitlement }) {
  ...
  const founderNumber = founderStatus?.founder_number
  const label = founderNumber ? `Founder #${String(founderNumber).padStart(2, '0')}` : active ? membershipEntitlement.plan_label : 'Steel Alpha'
  const status = founderNumber ? 'FOUNDER' : active ? 'ACTIVE' : 'ALPHA'
  return <span className={`profile-membership-badge ${founderNumber ? 'is-founder' : active ? 'is-active' : ''}`} aria-label={`Membership: ${label}, ${status}`}><ShieldCheck size={13}/><span><strong>{label}</strong><small>{status}</small></span></span>
}
...
function AccountSecurityPanel() {
  ...
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  async function refreshFactors() { try { setFactors(await getMfaFactors()) } catch { setFactors([]) } }
  useEffect(() => { refreshFactors() }, [])
  async function startAuthenticatorSetup() { setBusy(true); setError(''); try { setEnrollment(await enrollAuthenticatorApp()) } catch (reason) { setError(reason.message || 'Authenticator setup is unavailable. Check your security settings and try again.') } finally { setBusy(false) } }
  async function confirmAuthenticator() { if (!enrollment?.id || !/^\d{6}$/.test(code.trim())) { setError('Enter the 6-digit code from your authenticator app.'); return } setBusy(true); setError(''); try { await verifyAuthenticatorApp({ factorId: enrollment.id, code }); setEnrollment(null); setCode(''); await refreshFactors() } catch (reason) { setError(reason.message || 'That code could not be verified. Try the current code from your app.') } finally { setBusy(false) } }
  async function removeFactor(factorId) { if (!window.confirm('Remove this authenticator from your Steel account?')) return; setBusy(true); setError(''); try { await removeMfaFactor(factorId); await refreshFactors() } catch (reason) { setError(reason.message || 'We could not remove that authenticator.') } finally { setBusy(false) } }
  const verifiedAuthenticator = factors.find((factor) => factor.factor_type === 'totp' && factor.status === 'verified')
  return <section className="account-security-panel"><div className="settings-subsection-heading"><span className="eyebrow">TWO-STEP VERIFICATION</span><strong>Protect your account</strong></div><p className="settings-form-copy">Use an authenticator app now. Text-message verification and biometric/passkey sign-in will be introduced with Steel’s native mobile app.</p>{error && <p className="meal-component-save-error" role="alert">{error}</p>}{verifiedAuthenticator ? <div className="security-factor-row"><div><strong>Authenticator app</strong><small>Enabled · {verifiedAuthenticator.friendly_name || 'Project Steel'}</small></div><button type="button" className="text-link" disabled={busy} onClick={() => removeFactor(verifiedAuthenticator.id)}>Remove</button></div> : enrollment?.totp ? <div className="totp-enrollment"><img src={enrollment.totp.qr_code} alt="Scan this QR code with your authenticator app"/><div><strong>Scan the code, then confirm</strong><p>If scanning is unavailable, enter this secret in your app: <code>{enrollment.totp.secret}</code></p><label><span>6-digit code</span><input inputMode="numeric" autoComplete="one-time-code" maxLength="6" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ''))}/></label><div><button type="button" className="gold-button" disabled={busy} onClick={confirmAuthenticator}>{busy ? 'Checking…' : 'Confirm authenticator'}</button><button type="button" className="text-link" disabled={busy} onClick={() => { setEnrollment(null); setCode(''); setError('') }}>Cancel</button></div></div></div> : <button type="button" className="settings-security-action" disabled={busy} onClick={startAuthenticatorSetup}>Set up an authenticator app <ChevronRight size={15}/></button>}<div className="security-coming-soon"><div><span>SMS verification</span><small>Native app delivery setup required</small><em>Planned</em></div><div><span>Biometric / passkey</span><small>Face ID, Touch ID and Android biometrics</small><em>Planned</em></div></div></section>
}

function FeedbackButton({ onClick, className = '' }) {
  return <button type="button" className={`feedback-trigger ${className}`.trim()} onClick={onClick} aria-label="Provide feedback" title="Provide feedback"><MessageSquare size={15}/><span>Provide feedback</span></button>
}

function FeedbackForm({ category, setCategory, feedbackText, setFeedbackText, feedbackSaved, feedbackSaving, onSubmit }) {
  return <form className="support-feedback-form" onSubmit={onSubmit}><label htmlFor="feedback-category">Feedback type</label><select id="feedback-category" value={category} onChange={(event) => setCategory(event.target.value)}><option value="bug">Bug / something broke</option><option value="friction">Confusing / hard to use</option><option value="feature_request">Feature request</option><option value="training">Training</option><option value="nutrition">Nutrition</option><option value="progress_recovery">Progress / recovery</option><option value="other">Other</option></select><label htmlFor="feedback-message">What happened — or what would you change?</label><textarea id="feedback-message" value={feedbackText} onChange={(event) => setFeedbackText(event.target.value)} rows="4" maxLength="4000" required/>{feedbackSaved && <p className="support-feedback-success">Feedback received. Thank you for helping build Steel.</p>}<button className="gold-button" type="submit" disabled={feedbackSaving || !feedbackText.trim()}>{feedbackSaving ? 'Sending…' : 'Send feedback'}</button></form>
}

function SettingsPage({ user, userRole, onSignOut, closeSettings, accountName, avatarUrl, profileName, setProfileName, profileEmail, setProfileEmail, profilePhone, setProfilePhone, saveAccount, handleAvatar, avatarBusy, currentPassword, setCurrentPassword, newPassword, setNewPassword, confirmPassword, setConfirmPassword, savePassword, saving, preferences, setPreferences, toggleEquipment, savePreferences, openSupportPanel, onResetOnboarding, notificationPreferences, onRemindersSaved, founderStatus, membershipEntitlement, feedbackProps, planChangeStatus, onRequestPlanReview, activityConnections, onConnectActivityProvider, onDisconnectActivityProvider, onDeleteActivityProviderData }) {
  ...
  return <div className="page-stack settings-page-v5">
    <div className="settings-top-actions"><div className="settings-top-left"><button className="settings-back-button" type="button" onClick={closeSettings}><ArrowLeft size={17}/> Back</button><FeedbackButton onClick={() => openSupportPanel('feedback')}/></div><button className="settings-signout-button" type="button" onClick={onSignOut}><LogOut size={15}/> Sign out</button></div>
    <section className="page-intro"><span className="eyebrow">PROFILE & SETTINGS</span><h2>Your Steel profile</h2><button className="settings-support-jump" type="button" onClick={() => document.getElementById('settings-support-v5')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}><span>Need help?</span><strong>Support &amp; feedback</strong><ChevronRight size={15}/></button></section>
    <article className="settings-profile-card editable-profile"><div className="avatar-upload-wrap"><Avatar url={avatarUrl} size={72}/><label className="avatar-upload-button"><Camera size={16}/><input type="file" accept="image/jpeg,image/png,image/webp" onChange={handleAvatar} disabled={avatarBusy}/></label></div><div><span className="eyebrow">DISPLAY PROFILE</span><h3>{accountName}</h3><p>{avatarBusy ? 'Uploading photo…' : user.email}</p></div><ProfileMembershipBadge founderStatus={founderStatus} membershipEntitlement={membershipEntitlement}/></article>
    <SettingsDisclosure id="settings-profile-v5" eyebrow="PERSONAL PROFILE" title="Personal profile" icon={UserRound} open={profileOpen} onToggle={() => setProfileOpen(open => !open)}><form className="personal-profile-form" onSubmit={saveAccount}><div className="settings-subsection"><div className="settings-subsection-heading"><span className="eyebrow">CONTACT DETAILS</span><strong>Personal details</strong></div><div className="profile-form-grid"><label><span>Name</span><input value={profileName} onChange={e => setProfileName(e.target.value)} required /></label><label><span>Email</span><input type="email" value={profileEmail} onChange={e => setProfileEmail(e.target.value)} required /></label><label><span>Phone number</span><input type="tel" autoComplete="tel" placeholder="Optional" value={profilePhone} onChange={e => setProfilePhone(e.target.value)} /></label></div></div><div className="settings-subsection"><div className="settings-subsection-heading"><span className="eyebrow">ACCOUNT SECURITY</span><strong>Change password</strong></div><p className="settings-form-copy">Leave these blank if you only want to update your profile details.</p><label><span>Current password</span><input type="password" autoComplete="current-password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} /></label><label><span>New password</span><input type="password" autoComplete="new-password" minLength="6" value={newPassword} onChange={e => setNewPassword(e.target.value)} /></label><label><span>Confirm new password</span><input type="password" autoComplete="new-password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} /></label></div><button className="gold-button" disabled={saving}><Save size={17}/> {saving ? 'Saving…' : 'Save profile'}</button></form></SettingsDisclosure>
    <SettingsDisclosure id="settings-membership-v5" eyebrow="MEMBERSHIP" title="Membership & access" icon={ShieldCheck} open={membershipOpen} onToggle={() => setMembershipOpen(open => !open)}><MembershipStatusCard founderStatus={founderStatus} membershipEntitlement={membershipEntitlement}/></SettingsDisclosure>
    <SettingsDisclosure id="settings-security-v5" eyebrow="ACCOUNT SECURITY" title="Two-step verification" icon={ShieldCheck} open={securityOpen} onToggle={() => setSecurityOpen(open => !open)}><AccountSecurityPanel /></SettingsDisclosure>
    <SettingsDisclosure id="settings-preferences-v5" eyebrow="TRAINING PREFERENCES" title="Shape your Steel plan" icon={Target} open={preferencesOpen} onToggle={() => setPreferencesOpen(open => !open)}><PreferencesForm preferences={preferences} setPreferences={setPreferences} toggleEquipment={toggleEquipment} onSubmit={savePreferences} saving={saving} planChangeStatus={planChangeStatus} onRequestPlanReview={onRequestPlanReview}/></SettingsDisclosure>
    <HealthIntegrations connections={activityConnections} onConnect={onConnectActivityProvider} onDisconnect={onDisconnectActivityProvider} onDeleteImportedData={onDeleteActivityProviderData}/>
    <ReminderSettings userId={user.id} initialPreferences={notificationPreferences} onSaved={onRemindersSaved}/>
    {showAdminControls && <SettingsDisclosure id="settings-admin-v5" eyebrow="ADMIN CONTROLS" title="Backend / Admin" icon={ShieldCheck} open={adminOpen} onToggle={() => setAdminOpen(open => !open)}><div className="settings-admin-panel"><div><strong>Testing controls</strong><p>Owner-only tools for verifying onboarding and account flows.</p></div><button type="button" className="settings-admin-button" onClick={onResetOnboarding} disabled={saving}>{saving ? 'Resetting…' : 'Reset onboarding'}</button></div></SettingsDisclosure>}
    <section className="settings-support-card settings-support-footer" id="settings-support-v5"><div className="settings-footer-layout"><div className="settings-footer-heading"><div className="section-heading"><div><span className="eyebrow">NEED A HAND?</span><h3>Support</h3></div></div><div className="settings-support-list"><button type="button" onClick={() => openSupportPanel('help')}><span><strong>Help &amp; Support</strong><small>Get help using Steel</small></span><ChevronRight size={17}/></button><button type="button" onClick={() => openSupportPanel('about')}><span><strong>About Steel</strong><small>Learn more about the app</small></span><ChevronRight size={17}/></button><button type="button" onClick={() => openSupportPanel('feedback')}><span><strong>Send feedback</strong><small>Tell us how Steel can improve</small></span><ChevronRight size={17}/></button></div></div><div className="settings-footer-cards"><article className="settings-security-card"><span className="settings-security-icon"><ShieldCheck size={22}/></span><div><span className="eyebrow">YOUR DATA</span><h3>Private by default</h3><p>Workout, weight, profile and step records remain scoped to your authenticated account.</p></div></article><article className="settings-whats-new-card"><span className="eyebrow">YOU ASKED. WE LISTENED.</span><h3>What’s New</h3>{feedbackProps.updatesLoading ? <p>Loading updates…</p> : feedbackProps.productUpdates.length ? <div className="settings-update-list">{feedbackProps.productUpdates.map((update) => <article key={update.id}><span>{update.category || 'UX'}{update.from_feedback ? ' · FROM TESTERS' : ''}</span><strong>{update.title}</strong><p>{update.description}</p><small>{!update.read ? 'NEW · ' : ''}{formatDate(update.published_at)}</small></article>)}</div> : <p>Improvements informed by testers will appear here.</p>}</article></div></div></section>
  </div>
}
...
export default function AppV3({ user, onSignOut }) {
  ...
  const [activityConnections, setActivityConnections] = useState([])
  ...
  const [preferences, setPreferences] = useState({ goal: 'Lose fat and gain muscle', experienceLevel: 'Intermediate', availableEquipment: ['Machines'], trainingDays: 3, checkinDay: 0, units: 'lb', dailyStepGoal: 10000, limitations: '', dietaryPreference: 'No preference', allergies: '', mealsPerDay: 3, snacksEnabled: true, snackPreferences: [], goalTimeframeWeeks: 12, sessionDurationMin: 45, trainingLocation: 'Gym', currentTrainingDays: '', dailyActivityLevel: '', sleepQuality: '', trainingStyles: [], exercisePreferences: '', exerciseAvoidances: '', cardioPreference: 'No preference', cardioExperience: 'Beginner', cardioSessions: 0, cookingTime: '', preferredFoods: '' })
  ...
  const [membershipEntitlement, setMembershipEntitlement] = useState(null)
  const [planChangeStatus, setPlanChangeStatus] = useState(null)
  const [productUpdates, setProductUpdates] = useState([])
  const [updatesLoading, setUpdatesLoading] = useState(false)
  const [coachRelationships, setCoachRelationships] = useState([])
  ...
  useEffect(() => {
    ...
    getMyMembershipEntitlement().then(setMembershipEntitlement).catch(() => setMembershipEntitlement(null))
    getMyPlanChangeStatus().then(setPlanChangeStatus).catch(() => setPlanChangeStatus(null))
  }, [user.id])
  ...
  function refreshCoachRelationships() {
    setCoachLoading(true)
    loadCoachRelationships().then(setCoachRelationships).catch(() => setCoachRelationships([])).finally(() => setCoachLoading(false))
  }
  ...
  const stepGoal = stepGoalProgress(todaySteps, Number(preferences.dailyStepGoal) || 10000)
  const weeklySteps = sevenDayStepAverage(stepHistory)
  ...
  async function handleDisconnectActivityProvider(provider) {
    await disconnectActivityProvider(user.id, provider)
  ...
  async function handleDeleteActivityProviderData(provider) {
    await deleteActivityProviderData(provider)
    await Promise.all([refreshSteps(), getActivityConnections(user.id).then(setActivityConnections)])
  }
  ...
  async function persistPreferences(nextPreferences = preferences, { generatePlan = false } = {}) {
    ...
    try {
      ...
      return true
    } catch (e) { setMessage(e.message); return false } finally { setSaving(false) }
  }
  ...
  async function requestProgrammeReview() {
    if (!planChangeStatus?.can_change_now) return
    const updated = await persistPreferences(preferences, { generatePlan: true })
    if (updated) {
      await getMyPlanChangeStatus().then(setPlanChangeStatus).catch(() => {})
      setMessage('Your programme has been refreshed. Your next review opens in 28 days.')
    }
  }
  ...
  return <div className={`steel-app ${desktopNavCollapsed ? 'desktop-nav-collapsed' : ''}`}>
    <main className="steel-screen">
      <header className="v2-topbar">
        ...
        <div className="topbar-actions"><FeedbackButton onClick={() => openSupportPanel('feedback')}/><button className={`account-button ${tab === 'Settings' ? 'active' : ''}`} onClick={openSettings}><Avatar url={avatarUrl} size={32} /><span className="account-copy"><strong>{accountName}</strong><small>Profile</small></span><ChevronRight size={15} /></button></div>
      </header>
      ...
      {tab === 'Home' && <div className="page-stack home-page-stack">
        <section className="v4-welcome-card"><div className="v4-hero-art"><div className="v4-hero-figure" aria-hidden="true" /></div><div className="v4-hero-copy"><span className="eyebrow">{dailySummary.eyebrow}</span><h2>{firstName}</h2><p className="daily-motivation">{dailyQuote()}</p></div><div className="v4-metric-grid"><article><Dumbbell size={17}/><span>WORKOUTS</span><strong>{stats.sessionCount}</strong><small>Logged</small></article><article><Flame size={17}/><span>STREAK</span><strong>{stats.streakDays || 0}</strong><small>Days</small></article><article><Footprints size={17}/><span>STEPS</span><strong>{todaySteps.toLocaleString('en-GB')}</strong><small>Today</small></article><button type="button" className="v4-metric-link" onClick={() => navigateToTab('Weight')}><Scale size={17}/><span>WEIGHT</span><strong>{latestWeight ? latestWeight.toFixed(1) : '—'}</strong><small>lb</small></button></div></section>
        ...
        <HomeDirectionCard summary={dailySummary} recommendation={trainingRecommendation} hasWorkout={workouts.length > 0} onStartWorkout={() => openWorkout(workouts[0])} />
        <AiCoachInsightCard insight={aiCoachInsight} onNavigate={navigateToTab} hasWorkout={workouts.length > 0} />
        <PersonalisedJourneyCard workouts={workouts} preferences={preferences} navigateToTab={navigateToTab} onStartWorkout={openWorkout}/>
        <section className={`steel-card movement-card ${stepHistory.length ? '' : 'is-empty'}`}><div className="movement-card-heading"><div><span className="eyebrow">MOVEMENT HISTORY</span><h3>Steps total</h3><span className="metric-label">Last 30 days</span></div><div className="step-total"><strong>{totalSteps.toLocaleString('en-GB')}</strong><small>steps logged</small></div></div><div className="step-goal-summary"><div><strong>{stepGoal.completed}%</strong><span>of today’s {stepGoal.goal.toLocaleString('en-GB')} step goal</span></div><div className="step-goal-track" aria-hidden="true"><span style={{ width: `${stepGoal.completed}%` }}/></div></div><div className="movement-card-meta"><span>Today <strong>{todaySteps.toLocaleString('en-GB')}</strong>{steps.source === 'manual' ? ' · entered manually' : steps.source ? ` · synced from ${steps.source}` : ''}{stepGoal.remaining ? ` · ${stepGoal.remaining.toLocaleString('en-GB')} to go` : ' · goal reached'}</span><button className="text-link" onClick={() => navigateToTab('Progress')}>View progress <ChevronRight size={14}/></button></div><div className="movement-average"><strong>{weeklySteps.average.toLocaleString('en-GB')}</strong><span>7-day logged average · {weeklySteps.daysLogged} day{weeklySteps.daysLogged === 1 ? '' : 's'} logged</span></div><StepsChart data={stepHistory}/><ManualSteps userId={user.id} steps={steps} onSaved={refreshSteps}/>{!stepHistory.length&&<button className="movement-empty-link" onClick={openSettings}><Settings size={14}/> Explore planned health integrations</button>}</section>
...
{tab === 'Train' && <div className="page-stack training-page">{!selectedWorkout || !draft ? <section className="empty-state"><Dumbbell size={34}/><h2>Choose a workout</h2><p>Start from Home or Workouts.</p>{workouts[0]&&<button className="gold-button" onClick={()=>openWorkout(workouts[0])}>Start workout</button>}</section> : <><section className="train-top"><div><span className="eyebrow">ACTIVE WORKOUT</span><h2>{selectedWorkout.name}</h2></div><select value={selectedWorkout.id} onChange={(e)=>changeWorkout(e.target.value)}>{workouts.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></section><div className="train-progress-copy"><span>{atFinisher?'Final step':`Exercise ${draft.step+1} of ${activeExercises.length}`}</span><span>{Math.round(((draft.step+1)/(activeExercises.length+1))*100)}%</span></div><div className="train-progress"><span style={{width:`${Math.round(((draft.step+1)/(activeExercises.length+1))*100)}%`}}/></div>{!atFinisher&&currentExercise&&<><article className="exercise-focus-card"><div className="exercise-visual"><Dumbbell size={48}/><span>NOW</span></div><div className="exercise-focus-copy"><div className="exercise-focus-heading"><div><span className="eyebrow">CURRENT EXERCISE</span><h2>{currentExercise.name}</h2></div><span className="exercise-step-count">{draft.sets[currentExercise.id]?.filter((set) => set.complete && !set.removed).length || 0} / {currentExercise.sets}</span></div><p>{currentExercise.equipment} · {currentExercise.sets} sets × {currentExercise.reps}</p>{(currentExercise.muscleGroup||currentExercise.difficulty)&&<div className="exercise-metadata">{currentExercise.muscleGroup&&<span>{currentExercise.muscleGroup}</span>}{currentExercise.secondaryMuscleGroups?.slice(0,2).map(group=><span key={group}>{group}</span>)}{currentExercise.difficulty&&<span>{currentExercise.difficulty}</span>}</div>}{currentExercise.instructions&&<p className="exercise-instructions">{currentExercise.instructions}</p>}<div className="exercise-actions"><button type="button" className="exercise-action-button" onClick={openExercisePicker}><RotateCcw size={15}/> Change exercise</button>{currentExercise.youtubeUrl&&<a className="exercise-action-button exercise-video-link" href={currentExercise.youtubeUrl} target="_blank" rel="noreferrer"><ExternalLink size={15}/> Form video</a>}<button type="button" className="exercise-action-button exercise-remove-button" onClick={()=>setRemoveConfirmId(currentExercise.id)}><Trash2 size={15}/> Remove</button></div>{removeConfirmId===currentExercise.id&&<div className="exercise-remove-confirm"><span>Remove this exercise from today’s session?</span><div><button type="button" onClick={()=>setRemoveConfirmId(null)}>Keep it</button><button type="button" className="confirm-remove-button" onClick={confirmRemoveExercise}>Remove exercise</button></div></div>}</div></article><div className="set-list-v2">{(draft.sets[currentExercise.id]||[]).map(set=><article className={`set-row-v2 ${set.complete?'complete':''} ${set.removed?'removed':''}`} key={set.setNo}><div className="set-number">{set.setNo}</div>{!set.removed?<><label><span>KG</span><input type="number" min="0" step="2.5" value={set.weight} onChange={e=>updateSet(currentExercise.id,set.setNo,{weight:Number(e.target.value)||0})}/></label><label><span>REPS</span><input type="number" min="1" value={set.reps} onChange={e=>updateSet(currentExercise.id,set.setNo,{reps:Number(e.target.value)||1})}/></label><div className="set-actions-v2"><button className={set.complete?'done-button done':'done-button'} onClick={()=>updateSet(currentExercise.id,set.setNo,{complete:!set.complete})}><Check size={16}/> {set.complete?'Done':'Complete'}</button><button className="remove-button" onClick={()=>updateSet(currentExercise.id,set.setNo,{removed:true,complete:false})}><Trash2 size={15}/> Remove</button></div></>:<div className="removed-copy"><span>Removed</span><button onClick={()=>updateSet(currentExercise.id,set.setNo,{removed:false})}><RotateCcw size={15}/> Restore</button></div>}</article>)}</div><div className="train-nav-row"><button disabled={draft.step===0} onClick={()=>moveStep(draft.step-1)}><ArrowLeft size={17}/> Previous</button><button className="gold-button" onClick={()=>moveStep(draft.step+1)}>{draft.step===activeExercises.length-1?'Finisher':'Next'} <ArrowRight size={17}/></button></div></>}{atFinisher&&<><article className="finisher-panel"><div className="finisher-symbol"><Flame size={24}/></div><div><span className="eyebrow">FINISH STRONG</span><h2>Incline cardio</h2><p>7 min · 6% incline · RPE 6</p></div></article><article className="session-summary-v2"><div className="summary-hero"><Check size={20}/><div><span className="eyebrow">SESSION SUMMARY</span><strong>{completedSets} working sets</strong></div></div></article>{draft.removedExercises.length>0&&<details className="options-panel"><summary>Restore removed exercises</summary>{draft.removedExercises.map(id=>{const e=selectedWorkout.exercises.find(x=>x.id===id);return e?<button className="restore-exercise" key={id} onClick={()=>restoreExercise(id)}><RotateCcw size={14}/> {e.name}</button>:null})}</details>}<button type="button" className="gold-button add-exercise-from-session" onClick={openExercisePicker}><Dumbbell size={17}/> Add or change exercise</button><div className="train-nav-row"><button onClick={()=>moveStep(Math.max(activeExercises.length-1,0))}><ArrowLeft size={17}/> Back</button><button className="gold-button" disabled={!completedSets||saving} onClick={saveSession}><Save size={17}/> {saving?'Saving…':'Save session'}</button></div></>}</>}</div>}
      ...
      {tab === 'Settings' && <SettingsPage user={user} userRole={userRole} onSignOut={onSignOut} closeSettings={closeSettings} accountName={accountName} avatarUrl={avatarUrl} profileName={profileName} setProfileName={setProfileName} profileEmail={profileEmail} setProfileEmail={setProfileEmail} profilePhone={profilePhone} setProfilePhone={setProfilePhone} saveAccount={savePersonalProfile} handleAvatar={handleAvatar} avatarBusy={avatarBusy} currentPassword={currentPassword} setCurrentPassword={setCurrentPassword} newPassword={newPassword} setNewPassword={setNewPassword} confirmPassword={setConfirmPassword} savePassword={savePassword} saving={saving} preferences={preferences} setPreferences={setPreferences} toggleEquipment={toggleEquipment} savePreferences={savePreferences} openSupportPanel={openSupportPanel} onResetOnboarding={handleResetOnboarding} notificationPreferences={profile?.notification_preferences} onRemindersSaved={setProfile} founderStatus={founderStatus} membershipEntitlement={membershipEntitlement} planChangeStatus={planChangeStatus} onRequestPlanReview={requestProgrammeReview} activityConnections={activityConnections} onConnectActivityProvider={handleConnectActivityProvider} onDisconnectActivityProvider={handleDisconnectActivityProvider} onDeleteActivityProviderData={handleDeleteActivityProviderData} feedbackProps={{ category: feedbackCategory, setCategory: setFeedbackCategory, feedbackText, setFeedbackText, feedbackSaved, feedbackSaving, onSubmit: saveFeedback, productUpdates, updatesLoading }}/>}
