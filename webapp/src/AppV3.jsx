...
import { stepGoalProgress } from './lib/steps'
import './app-v2.css'
import './settings.css'
import './v3.css'
...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  return <div className="page-stack nutrition-page">
    ...
    {nutritionView === 'diary' && <FoodDiary userId={userId} mealLogs={loggedMeals} onChanged={refreshLoggedMeals} />}
    {nutritionView === 'plan' && <section className="nutrition-plan-summary"><div><span className="eyebrow">TODAY’S PLAN</span><strong>{plannedLogs.length} of {mealOrder.length} planned meals logged</strong></div><button type="button" className="text-link" onClick={() => { setEditingCustomGroup(null); setCustomOpen((open) => !open) }}>{customOpen ? 'Close quick add' : 'Add custom meal'} <ChevronRight size={14}/></button></section>}
