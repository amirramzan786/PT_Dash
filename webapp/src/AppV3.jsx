...
import {
  ...
  loadExerciseCatalog, loadWorkouts, saveCustomWorkout, saveProfile, saveWeight, saveWorkoutSession, updateAccount, uploadAvatar,
} from './lib/steelApi'
import './app-v2.css'
import './settings.css'
import './v3.css'
...
function ExerciseLibrary() {
  ...
  return <section className="exercise-library"><div className="library-toolbar"><label className="library-search"><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search exercises" aria-label="Search exercises"/></label><div className="library-filters"><select value={muscleGroup} onChange={(event) => setMuscleGroup(event.target.value)} aria-label="Filter by muscle group"><option>All muscle groups</option>{muscles.map((muscle) => <option key={muscle}>{muscle}</option>)}</select><select value={equipment} onChange={(event) => setEquipment(event.target.value)} aria-label="Filter by equipment"><option>All equipment</option>{equipmentOptions.map((item) => <option key={item}>{item}</option>)}</select></div></div>{busy ? <div className="library-state">Loading the exercise library…</div> : error ? <div className="library-state library-error">The exercise library is unavailable right now. Try again shortly.</div> : <><div className="library-result-count">{visibleRows.length} exercise{visibleRows.length === 1 ? '' : 's'} available</div><div className="exercise-library-list">{visibleRows.map((row) => <article className="exercise-library-card" key={row.id}><div className="library-exercise-icon"><Dumbbell size={19}/></div><div className="library-exercise-copy"><strong>{row.name}</strong><span>{row.primary_muscle_group} · {(row.equipment ?? []).join(' / ') || 'Gym'}</span><small>{row.difficulty} · {row.movement_pattern}</small></div>{row.video_url ? <a className="library-video-link" href={row.video_url} target="_blank" rel="noreferrer">Form <ExternalLink size={13}/></a> : null}</article>)}{!visibleRows.length && <div className="library-state">No exercises match those filters.</div>}</div></>}</section>
}

function LogWorkoutPage({ onCancel, onStart, saving }) {
...
export default function AppV3({ user, onSignOut }) {
  ...
  async function openExercisePicker() {
    setExercisePickerOpen(true); setExercisePickerBusy(true); setMessage('')
    try { setExerciseOptions(await loadExerciseCatalog({ muscleGroup: currentExercise?.muscleGroup, limit: 24 })) }
    catch (e) { setMessage(e.message) } finally { setExercisePickerBusy(false) }
  }
  ...
  return <div className="steel-app">
    <main className="steel-screen">
      ...
      {tab === 'Plan' && (customLogOpen ? <LogWorkoutPage saving={saving} onCancel={() => setCustomLogOpen(false)} onStart={saveAndStartCustomWorkout}/> : <PlanPage workouts={workouts} openWorkout={openWorkout} onLogWorkout={() => setCustomLogOpen(true)}/>)}
