  ...
  return <section className="today-meal-plan">
    ...
    {mealOrder.map((meal) => {
      ...
      return <article className={existing ? 'today-meal-card is-logged' : 'today-meal-card'} key={meal}>
        ...
        {finder?.meal === meal && <section className="inline-food-finder today-meal-food-finder"><div><span className="eyebrow">ADD TO {meal}</span><h3>Find a food</h3><p>Search the catalogue or scan a barcode, then choose the serving you actually had.</p></div><FoodDiary userId={finder.userId} mealLogs={[]} initialMeal={meal} onFoodPicked={finder.onFoodPicked}/><button type="button" className="text-link" onClick={finder.onClose}>Close food finder</button></section>}
      </article>
    })}
  </section>
}
