  ...
  return <div className="steel-app">
    <main className="steel-screen">
      ...
      {tab === 'Home' && <div className="page-stack">
        <section className="welcome-row v3-welcome"><div><div className="eyebrow">WELCOME BACK</div><h2>{firstName}, let’s build.</h2></div><div className="top-stats"><div className="mini-stat"><Flame size={18}/><strong>{stats.sessionCount}</strong><span>Sessions</span></div><div className="mini-stat"><Footprints size={18}/><strong>{Number(steps.steps || 0).toLocaleString('en-GB')}</strong><span>Today’s steps</span></div></div></section>
        <section className="v4-welcome-card"><div className="v4-hero-art" aria-hidden="true"><div className="v4-hero-figure" /></div><div className="v4-hero-copy"><span className="eyebrow">TODAY’S FOCUS</span><h2>Build strength.<br/>Build consistency.</h2><p>Your next session is ready when you are.</p></div></section>
        <section className="v4-metric-grid"><article><Dumbbell size={17}/><span>SESSIONS</span><strong>{stats.sessionCount}</strong><small>Logged</small></article><article><Flame size={17}/><span>STREAK</span><strong>—</strong><small>Needs history</small></article><article><Scale size={17}/><span>VOLUME</span><strong>—</strong><small>Needs history</small></article><article><Check size={17}/><span>WEIGHT</span><strong>{latestWeight?latestWeight.toFixed(1):'—'}</strong><small>lb · latest</small></article></section>
        ...
        <section className="hero-panel"><div className="hero-copy"><span className="gold-kicker">TODAY’S FOCUS</span><h2>Build strength.<br/>Build consistency.</h2><p>Your plan, progress and daily movement in one place.</p>{workouts[0] && <button className="gold-button" onClick={() => openWorkout(workouts[0])}><Play size={17}/> Start workout</button>}</div><div className="hero-art"><Dumbbell size={64}/></div></section>
