...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  return <div className={`page-stack nutrition-page nutrition-page-${nutritionView}`}>
    ...
    {(nutritionView === 'diary' || nutritionView === 'plan') && <article className="nutrition-hero"><div className="macro-orb"><span>{loggedCalories.toLocaleString('en-GB')}</span><small>kcal logged</small></div><div><span className="eyebrow">TODAY’S TARGET</span><h3>{target.calories.toLocaleString('en-GB')} kcal · {target.protein_g}g protein</h3><p>{dietary} · {preferences.mealsPerDay} meals per day{preferences.snacksEnabled !== false ? ' + snacks' : ''}</p></div></article>}
