...
function NutritionPage({ preferences, navigateToTab, userId }) {
  ...
  const [customForm, setCustomForm] = useState({ meal: 'SNACK', name: '', calories: '', protein: '', carbs: '', fat: '', servingG: '' })
  ...
  useEffect(() => { loadPlan(); getMealLogs(userId, today, today).then((rows) => setLoggedMeals(rows)).catch(() => {}) }, [userId])
  ...
  async function removeMeal(log) {
    ...
    try { await deleteMealLog({ userId, id: log.id }) } catch { setLoggedMeals((current) => [...current, log]) }
  }
  async function addCustomMeal(event) {
    event.preventDefault()
    if (!customForm.name.trim() || !customForm.calories) return
    const draft = { id: `draft-custom-${Date.now()}`, meal_date: today, meal_type: customForm.meal, entry_type: 'custom', recipe_name: customForm.name.trim(), calories: Number(customForm.calories) || 0, protein_g: Number(customForm.protein) || 0, carbs_g: Number(customForm.carbs) || 0, fat_g: Number(customForm.fat) || 0, serving_g: Number(customForm.servingG) || null, portion_multiplier: 1 }
    setLogBusy(true)
    setLoggedMeals((current) => [...current, draft])
    try {
      const saved = await saveMealLog({ userId, mealDate: today, mealType: draft.meal_type, entryType: 'custom', recipeName: draft.recipe_name, calories: draft.calories, protein: draft.protein_g, carbs: draft.carbs_g, fat: draft.fat_g, servingG: draft.serving_g })
      setLoggedMeals((current) => current.map((row) => row.id === draft.id ? saved : row))
      setCustomForm({ meal: 'SNACK', name: '', calories: '', protein: '', carbs: '', fat: '', servingG: '' })
      setCustomOpen(false)
    } catch { setLoggedMeals((current) => current.filter((row) => row.id !== draft.id)) } finally { setLogBusy(false) }
  }
  ...
  return <div className="page-stack nutrition-page">
    ...
    {nutritionView === 'plan' && <section className="nutrition-plan-summary"><div><span className="eyebrow">TODAY’S PLAN</span><strong>{plannedLogs.length} of {mealOrder.length} planned meals logged</strong></div><button type="button" className="text-link" onClick={() => setCustomOpen((open) => !open)}>{customOpen ? 'Close quick add' : 'Add unplanned meal'} <ChevronRight size={14}/></button></section>}
    {customOpen && <form className="nutrition-custom-form steel-card" onSubmit={addCustomMeal}><div><span className="eyebrow">QUICK ADD</span><h3>Log what you actually ate</h3><p>A meal outside the plan still counts toward today’s total.</p></div><div className="nutrition-custom-grid"><label><span>Meal</span><select value={customForm.meal} onChange={(event) => setCustomForm({ ...customForm, meal: event.target.value })}>{mealOrder.map((meal) => <option key={meal}>{meal}</option>)}</select></label><label><span>Food or meal name</span><input required value={customForm.name} onChange={(event) => setCustomForm({ ...customForm, name: event.target.value })} placeholder="e.g. Burger and fries" /></label><label><span>Calories</span><input required inputMode="numeric" type="number" min="1" value={customForm.calories} onChange={(event) => setCustomForm({ ...customForm, calories: event.target.value })} /></label><label><span>Protein (g)</span><input inputMode="numeric" type="number" min="0" value={customForm.protein} onChange={(event) => setCustomForm({ ...customForm, protein: event.target.value })} /></label><label><span>Carbs (g)</span><input inputMode="numeric" type="number" min="0" value={customForm.carbs} onChange={(event) => setCustomForm({ ...customForm, carbs: event.target.value })} /></label><label><span>Fat (g)</span><input inputMode="numeric" type="number" min="0" value={customForm.fat} onChange={(event) => setCustomForm({ ...customForm, fat: event.target.value })} /></label></div><button className="gold-button" disabled={logBusy}><Save size={16}/> Add to today</button></form>}
    <section className="nutrition-plan-list"><div className="section-heading"><div><span className="eyebrow">{nutritionView === 'plan' ? 'YOUR ASSIGNED MEALS' : 'RECIPE LIBRARY'}</span><h3>{nutritionView === 'plan' ? 'Today’s meals' : 'Browse recipes'}</h3></div><span className="nutrition-progress">3 choices each</span></div>{planBusy ? <div className="library-state">Loading your meal plan…</div> : mealOrder.length ? mealOrder.map((meal) => { const options = assignedRecipes.filter((recipe) => recipe.meal === meal); const selected = options.find((recipe) => recipe.optionKey === selectedChoices[meal]) || options[0]; const existing = plannedLogs.find((row) => row.meal_type === meal); const portion = Number(portions[meal] || 1); return <article className={`steel-card nutrition-meal-card ${existing ? 'is-logged' : ''} ${openMeal === meal ? 'is-expanded' : ''}`} key={meal}><div className="nutrition-meal-icon"><Salad size={19}/></div><div className="nutrition-recipe-copy"><span className="eyebrow">{meal}</span><h3>{selected.name}</h3><p>{selected.detail}</p><small>{Math.round(selected.calories * portion)} kcal · {Math.round(selected.protein * portion)}g protein · {Math.round(selected.carbs * portion)}g carbs · {Math.round(selected.fat * portion)}g fat</small></div><div className="nutrition-meal-actions"><button type="button" className="nutrition-details-button" onClick={() => setOpenMeal(openMeal === meal ? null : meal)}>{openMeal === meal ? 'Hide' : 'Choose'}</button><button type="button" className="nutrition-log-button" disabled={logBusy} onClick={() => logMeal(selected)}>{existing ? 'Update log' : 'Log meal'}</button></div>{openMeal === meal && <div className="nutrition-choice-panel"><div className="nutrition-choice-list">{options.map((option) => <button type="button" key={option.optionKey} className={selected.optionKey === option.optionKey ? 'active' : ''} onClick={() => setSelectedChoices({ ...selectedChoices, [meal]: option.optionKey })}><strong>{option.name}</strong><span>{option.calories} kcal · {option.protein}g protein</span></button>)}</div><div className="nutrition-portion-row"><label><span>Portion</span><select value={portion} onChange={(event) => setPortions({ ...portions, [meal]: Number(event.target.value) })}>{[[.5,'Half'],[.75,'Three quarters'],[1,'Full serving'],[1.25,'One and a quarter'],[1.5,'One and a half']].map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="recipe-detail"><strong>Ingredients</strong><span>{selected.ingredients.join(' · ')}</span><strong>How to make it</strong><span>{selected.instructions}</span></div></div></div>}</article> }) : <div className="library-state"><strong>No assigned meals yet.</strong><span>Complete your nutrition preferences in Settings so Steel can prepare your meal plan.</span><button type="button" className="library-retry" onClick={() => navigateToTab('Settings')}>Open Settings</button></div>}</section>
    {customLogs.length > 0 && <section className="nutrition-custom-log-list"><div className="section-heading"><div><span className="eyebrow">ADDED TODAY</span><h3>Unplanned meals</h3></div></div>{customLogs.map((log) => <article className="steel-card nutrition-custom-log" key={log.id}><div><span className="eyebrow">{log.meal_type}</span><strong>{log.recipe_name}</strong><small>{log.calories} kcal · {log.protein_g}g protein · {log.carbs_g}g carbs · {log.fat_g}g fat</small></div><button type="button" onClick={() => removeMeal(log)}><Trash2 size={15}/> Remove</button></article>)}</section>}
